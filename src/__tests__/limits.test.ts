...
describe('GaqlBuilder - Query Size Limits', () => {
  describe('SELECT field limits', () => {
    it('should reject queries exceeding maximum SELECT fields', () => {
      const fields = Array.from({ length: QUERY_LIMITS.MAX_SELECT_FIELDS + 1 }, (_, i) => `field${i}`);
      ...
      expect(() => {
        new GaqlBuilder()
          ...
          .from('campaign')
          .build();
      }).toThrow(`SELECT clause exceeds maximum field limit. Expected: <= ${QUERY_LIMITS.MAX_SELECT_FIELDS} fields, Received: ${fields.length} fields`);
    });
  ...
  describe('WHERE condition limits', () => {
    it('should reject queries exceeding maximum WHERE conditions', () => {
      ...
      }).toThrow(`WHERE clause exceeds maximum condition limit. Expected: < ${QUERY_LIMITS.MAX_WHERE_CONDITIONS} conditions, Received: ${QUERY_LIMITS.MAX_WHERE_CONDITIONS} conditions`);
    });
    ...
    it('should reject queries exceeding maximum parameters', () => {
      ...
      expect(() => {
        new GaqlBuilder()
          ...
          .parameters(params)
          .build();
      }).toThrow(`PARAMETERS clause exceeds maximum parameter limit. Expected: <= ${QUERY_LIMITS.MAX_PARAMETERS} parameters, Received: ${QUERY_LIMITS.MAX_PARAMETERS + 1} parameters`);
    });
  });
});